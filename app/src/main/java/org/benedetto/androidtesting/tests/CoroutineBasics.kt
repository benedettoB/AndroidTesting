...
object CoroutineBasics {
    ...
    val TAG = "CoroutineBasics"

    fun begin() = runBlocking {
        //firstCoroutine()
        launch { // launch a new coroutine and continue
            delay(1000L) // non-blocking delay for 1 second (default time unit is ms)
            log(TAG,"firstCoroutine() World!") // print after delay
        }
        log(TAG,"firstCoroutine() Hello") // main coroutine continues while a previous one is delayed

        //secondCoroutine()
        launch {
            secondCoroutine()
        }
        log(TAG," secondCoroutine() Hello")

        //thirdCoroutine()
        thirdCoroutine()
        log(TAG,"thirdCoroutine() Done")

        //fourthCoroutine()
        val job = launch { // launch a new coroutine and keep a reference to its Job
            delay(1000L)
            log(TAG,"fourthCoroutine() World!")
        }
        log(TAG,"fourthCoroutine() Hello")
        job.join() // wait until child coroutine completes
        log(TAG,"fourthCoroutine() Done")
    }
    ...
    fun fifthCoroutine() = runBlocking {
        val time = measureTimeMillis {
            repeat(10_000) { // launch a lot of coroutines
                launch {
                    delay(5000L)
                    log(TAG,"fifthCoroutine() ")
                }
            }
        }
        log(TAG,"fifthCoroutine() time to complete: $time")
    }
    ...
    fun threadingCompared(){
        val time = measureTimeMillis {
            repeat(10_000) { // launch a lot of threads
                thread {
                    Thread.sleep(5000L)
                    log(TAG,"threadingCompared() ")
                }
            }
        }
        log(TAG,"threadingCompared() time to complete: $time")
    }
    /*
    ...
    private suspend fun thirdCoroutine() = coroutineScope { // this: CoroutineScope
        launch {
            delay(2000L)
            log(TAG,"thirdCoroutine() World 2")
        }
        launch {
            delay(1000L)
            log(TAG,"thirdCoroutine() World 1")
        }
        log(TAG,"thirdCoroutine() Hello")
    }

    private suspend fun secondCoroutine() {
        delay(1000L)
        log(TAG,"secondCoroutine() World!")
    }
